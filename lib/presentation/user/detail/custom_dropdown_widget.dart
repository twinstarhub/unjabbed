...
class _CustomDropDownWidgetState<T> extends State<CustomDropDownWidget<T>>
  ...
  void initState() {
    ...
    _animationColor=ColorTween(begin: widget.primaryColor??Colors.white, end: widget.seconndaryColor??Colors.white)
    .animate(_animationController);
    _animationColorShadow=ColorTween(begin: Colors.transparent, end:Colors.blue.withOpacity(0.3))
    .animate(_animationController);
      ...
      child: InkWell(
        ...
        child: AnimatedBuilder(
          ...
          builder: (context,Widget?child) {
            return Container(
                    margin:widget.border==false? const EdgeInsets.fromLTRB(0, 0, 16, 0):null,
                    padding:const EdgeInsets.symmetric(vertical: 4, horizontal: 16),
                    // height: widget.height,
                    width:widget.width?? double.infinity,
                    decoration: BoxDecoration(
                      boxShadow: [if(_isOpen&&widget.shadow)BoxShadow(
                        color: _animationColorShadow.value??Colors.transparent,
                        spreadRadius: 3 ,
                        blurRadius: 0.1
                      )],
                      color:_animationColor.value,
                      border:widget.border? Border.all(color:_animationColorBorder.value??Colors.transparent):null,
                      borderRadius:widget.border? BorderRadius.circular(8):const BorderRadius.only(topLeft: Radius.circular(8),bottomLeft:Radius.circular(8) )
                    ),
            child: Row(
                    ...
                    Expanded(
                      child:TextFormField(
                        ...
                        onFieldSubmitted: (value)async{
                          ...
                          try {
                            final currentIndex=widget.lista.indexOf(filter.value[0]);
                            index=currentIndex;
                            widget.onChange(filter.value[0]);
                            _value=filter.value[0];
                            widget.textEditingController.text=filter.value[0].label;
                            filter.value=widget.lista;
                            await _toggleDropdown();
                          } catch (e) {
                            _value=null;
                        ...
                        onChanged: (value){
                          ...
                          filter.value=widget.lista.where((e) => e.label.toLowerCase().contains(value.toLowerCase())).toList();
                          setState(() {
                        ...
                        controller: widget.textEditingController,
                            validator: (value){
                              if(value==null || value.isEmpty||_value==null){
                                return '';
                              }else{
                                return null;
                              }
      ...
      builder: (context) => GestureDetector(
        ...
        child: ValueListenableBuilder<List<Vinculo>>(
          ...
          builder: (context,value,_) {
            ...
            bool isNegative=widget.withSearch?false: (heightScreen-(offset.dy+100+height)).isNegative;
            return SizedBox(
                  ...
                  children: [
                    Positioned(
                      ...
                      child: CompositedTransformFollower(
                        offset:
                          Offset(0,isNegative? size.height-45:size.height+5),
                        link: _layerLink,
                                      ...
                                      onTap: (){
                                        index=i;
                                              ...
                                              if(widget.valueVinculo){
                                                widget.textEditingController.text=item.value;
                                              }else{
                                              widget.textEditingController.text=item.label;
                                       ...
                                       child: Container(
                                         ...
                                         color: item==_value? widget.dark?Colors.blueAccent: Colors.white:null,
                                         child: Text(item.label.trim(),
  ...
  Future<void> _toggleDropdown({bool close = false}) async {
    if (_isOpen || close) {
      ...
      _controller=ScrollController(initialScrollOffset: ((index??0.0)*36+((index??0.0)*3)));
      }
